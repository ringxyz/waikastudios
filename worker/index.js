...
const CLEAN_ROUTES = new Map([
  ["/planes", "/planes.html"], ["/trabajos", "/trabajos.html"],
...
function publicOrigin(env) {
  return configuredPublicOrigin(env) || DEFAULT_ORIGIN;
}
...
function allowedPageRequest(request, env) {
  const allowedOrigin = publicOrigin(env);
...
async function rewriteSeoOrigin(response, pathname, origin) {
  ...
  const rewriteable = contentType.includes("text/html") || pathname === "/sitemap.xml" || pathname === "/robots.txt";
...
async function createChallenge(request, env) {
  if (!withinRateLimit(request)) return json({ message: "Demasiados intentos. Vuelve a probar en unos minutos." }, 429, { "Retry-After": "900" });
...
async function handleBriefing(request, env) {
  ...
  try {
    ...
    if (error instanceof RangeError) return json({ success: false, message: "La solicitud es demasiado grande." }, 413);
    return json({ success: false, message: "Solicitud no válida." }, 400);
  }
  if (!body || typeof body !== "object" || Array.isArray(body)) return json({ success: false, message: "Solicitud no válida." }, 400);

  if (clean(body.website, 200)) return json({ success: true });
  if (!withinRateLimit(request)) {
    return json({ success: false, message: "Demasiados intentos. Inténtalo de nuevo más tarde." }, 429, { "Retry-After": "900" });
  }
  ...
  const deliveryJob = inFlightSubmissions.get(idempotencyKey) || (async () => {
    try {
      ...
      if (!response.ok) {
        ...
        return { success: false, status: 502, message: "El briefing no pudo procesarse. Inténtalo de nuevo en unos minutos." };
      }
      return { success: true, status: 200, webhookAccepted: true };
    } catch (error) {
      console.error("Make delivery failed", { name: error?.name || "Error" });
      return { success: false, status: 502, message: "No pudimos enviar el briefing. Inténtalo de nuevo en unos minutos." };
    }
  })();
...
const CHAT_SYSTEM = `Eres el asistente web de Waika Studios. Responde en el idioma indicado por el visitante, de forma breve, amable y concreta. Solo puedes afirmar estos datos: Waika diseña webs, contenido, herramientas digitales, aplicaciones, IA y automatizaciones; una landing interactiva sin administrador cuesta 250 EUR; una versión con administrador para editar el contenido acordado cuesta 350 EUR; la gestión de redes sociales empieza en 250 EUR; asesorías y paquetes de contenido aún no tienen tarifa publicada; apps y software a medida se cotizan después de estudiar el caso. Los impuestos aplicables y condiciones se confirman en la propuesta. Para landings, la primera versión se entrega en 24-48 horas laborables cuando el pago, briefing y materiales necesarios están completos; si se incumple ese plazo, se devuelve el 50% según esas condiciones. No prometas fecha de publicación, resultado comercial, precios adicionales o condiciones no listadas. Para conocer alcance y propuesta invita a completar /onboarding.html. No pidas contraseñas, datos financieros ni información personal sensible. No eres soporte técnico de productos de terceros.`;

async function handleChat(request, env) {
  if (!allowedPageRequest(request, env)) return json({ success: false, message: "Origen no permitido." }, 403);
  if (!withinRateLimit(request)) return json({ success: false, message: "Has enviado varias consultas. Vuelve a intentarlo en unos minutos." }, 429, { "Retry-After": "900" });
  if (!env.ANTHROPIC_API_KEY) return json({ success: false, message: "El asistente está terminando de configurarse. Puedes dejar tu pregunta en el briefing." }, 503);
  if (!request.headers.get("Content-Type")?.toLowerCase().startsWith("application/json")) return json({ success: false, message: "Tipo de contenido no permitido." }, 415);
  ...
  try {
    ...
    return json({ success: false, message: large ? "La consulta es demasiado larga." : "Solicitud no válida." }, large ? 413 : 400);
  }
  const visitorMessages = Array.isArray(body?.messages)
    ...
    : [];
  if (!visitorMessages.length) return json({ success: false, message: "Escribe una pregunta para continuar." }, 400);
  ...
  try {
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({ model: clean(env.ANTHROPIC_MODEL, 100) || "claude-sonnet-5", max_tokens: 420, system: CHAT_SYSTEM, messages }),
      signal: controller.signal
    });
    clearTimeout(timer);
    if (!response.ok) {
      console.error("Anthropic chat failed", { status: response.status });
      return json({ success: false, message: "No pudimos responder ahora. Inténtalo de nuevo o continúa por el briefing." }, 502);
    }
    const data = await response.json();
    const answer = clean((data.content || []).filter((part) => part.type === "text").map((part) => part.text).join("\n"), 3000);
    return answer ? json({ success: true, answer }) : json({ success: false, message: "No llegó una respuesta. Prueba otra vez." }, 502);
  } catch (error) {
    clearTimeout(timer);
    console.error("Anthropic chat unavailable", { name: error?.name || "Error" });
    return json({ success: false, message: "El asistente no está disponible ahora. Déjanos tu pregunta en el briefing." }, 502);
  }
}
...
export default {
  async fetch(request, env) {
    ...
    const canonicalOrigin = configuredOrigin || DEFAULT_ORIGIN;
